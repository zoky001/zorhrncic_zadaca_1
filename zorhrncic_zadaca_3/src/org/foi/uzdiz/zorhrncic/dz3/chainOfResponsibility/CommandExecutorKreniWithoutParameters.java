 ...
 * @author Zoran
 */
public class CommandExecutorKreniWithoutParameters extends CommandExecutor {

    public CommandExecutorKreniWithoutParameters() {
        this.typeOfCommand = typeOfCommand.KRENI_BEZ_PARAMETRA;
    }
    ...
    protected DispecerContext executeCommandPrivate(Command command, DispecerContext dispecerContext) {
        ...
        this.builderDirector.addTitleInReport("Izvršavam komandu \"KRENI_BEZ_PARAMETRA\"..", true);
        this.builderDirector.addTitleInReport("Kreće preuzimanje otpada do kraja.", true);

        startCollecting();
        this.builderDirector.addTitleInReport("Završena komanda \"KRENI_BEZ_PARAMETRA\"..", true);
...
//        while (!context.isIsAllWasteCollected() || context.getAllVehiclesAtLandfill().size() > 0) {
...
//
//        }
        while ((!context.isIsAllWasteCollected() || context.getAllVehiclesAtLandfill().size() > 0 || context.getAllVehiclesAtGasStation().size() > 0)) {
            context.setIsAllWasteCollected(true);
